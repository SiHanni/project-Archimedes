...
⚠️ **fp32(`model.onnx`, 928MB) 만 쓸 수 있다.** fp16 은 리눅스 CPU 빌드의 ONNX
Runtime 에 `com.microsoft.Gelu` 의 float16 커널이 아예 없어 세션 생성부터
실패한다 — **1.29 도 1.28 도 마찬가지**라 버전을 내려도 소용없다(둘 다 실측).
맥 로컬(arm64 휠)에서는 같은 1.28 로 돌아가기 때문에 로컬 검증만으로는 절대
안 잡힌다. 반드시 컨테이너에서 확인할 것.

⚠️ **intra-op 스레드는 4 를 넘기지 말 것.** 8 로 올리면 워커가 잡 중간에 죽는다 —
`OOMKilled=false` · `ExitCode=0` 으로 찍혀 정상 종료처럼 보이고, 죽을 때마다
928MB 모델을 25초에 걸쳐 다시 읽는다. 그 사이 처리 중이던 잡은 유실돼
사용자에게는 **영원히 processing** 으로 보인다(실측 RestartCount=5, 잡 194초 미완).
`docker stats` 의 상주 사용량은 1.2GiB 로 낮게 보이는데, 순간 첨두는 그보다
훨씬 높아 이 지표로는 안전을 판단할 수 없다. 4 에서는 RestartCount=1 로 안정.
